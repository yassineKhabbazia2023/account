// <copyright file="MapAccountDatabaseToAccountModelTests.cs" company="Pulse">
...
public class MapAccountDatabaseToAccountModelTests
{
    private readonly Fixture _fixture;

    public MapAccountDatabaseToAccountModelTests()
    {
    ...
    {
        // arrange
        var deployment = _fixture.Create<DeploymentEntity>();
        var accountEntity = _fixture.Build<AccountEntity>()
    ...
    public void MapToUpdatedAccount_WithNullProperties_ShouldNotUpdateCorrespondingFields()
    {
        ...
        var accountDetail = new AccountDetail
        {
            AccountNumber = "T12345",
            Phone = new List<Phone> { new Phone { PhoneNumber = "0600000000" } },
            Legal = new Legal { LegalName = "Legal", StaffSizeRange = "Original" },
            Accounting = null,
            Deployment = null,
            Hub = null,
            Vat = null
    ...
    {
        // Arrange
        var entity = _fixture.Create<OfferEligibilityEntity>();
    ...
    [Fact]
    public void MapToOffice_Should_MapCorrectly()
    {
        var entity = _fixture.Create<OfficeEntity>();

        var result = entity.MapToOffice();

        result.Should().NotBeNull();
        result.OfficeId.Should().Be(entity.OfficeId);
        result.Name.Should().Be(entity.Name);
        result.PhoneNumber.Should().Be(entity.PhoneNumber);
        result.AddressId.Should().Be(entity.AddressId);

        var entityAddress = entity.Address;
        var resultAddress = result.Address;
        resultAddress.Should().NotBeNull();
        resultAddress.AddressId.Should().Be(entityAddress.AddressId);
        resultAddress.AddressLine1.Should().Be(entityAddress.AddressLine1);
        resultAddress.AddressLine2.Should().Be(entityAddress.AddressLine2);
        resultAddress.AddressLine3.Should().Be(entityAddress.AddressLine3);
        resultAddress.State.Should().Be(entityAddress.State);
        resultAddress.ZipCode.Should().Be(entityAddress.ZipCode);
        resultAddress.Country.Should().Be(entityAddress.Country);
        resultAddress.City.Should().Be(entityAddress.City);
        resultAddress.AddressType.Should().Be(entityAddress.AddressType);
        resultAddress.Latitude.Should().Be(entityAddress.Latitude);
        resultAddress.Longitude.Should().Be(entityAddress.Longitude);
    }

    [Fact]
    public void MapToOffice_WhenAddressIsNull_Should_ReturnDefaultAddress()
    {
        var entity = _fixture.Build<OfficeEntity>()
            .Without(o => o.Address)
            .Create();
    ...
    [Fact]
    public void MapToOffice_WhenSourceIsNull_Should_ReturnNull()
    {
        var result = MapAccountDatabaseToAccountModel.MapToOffice((OfficeEntity)null!);

        result.Should().BeNull();
    }

    [Fact]
    public void MapToOfficeEntity_Should_MapCorrectly()
    {
        var office = _fixture.Create<Office>();

        var result = office.MapToOffice();

        result.Should().NotBeNull();
        result.OfficeId.Should().Be(office.OfficeId);
        result.Name.Should().Be(office.Name);
        result.PhoneNumber.Should().Be(office.PhoneNumber);
        result.AddressId.Should().Be(office.AddressId);

        var address = office.Address!;
        var resultAddress = result.Address;
        resultAddress.Should().NotBeNull();
        resultAddress.AddressId.Should().Be(address.AddressId);
        resultAddress.AddressLine1.Should().Be(address.AddressLine1);
        resultAddress.AddressLine2.Should().Be(address.AddressLine2);
        resultAddress.AddressLine3.Should().Be(address.AddressLine3);
        resultAddress.State.Should().Be(address.State);
        resultAddress.ZipCode.Should().Be(address.ZipCode);
        resultAddress.Country.Should().Be(address.Country);
        resultAddress.City.Should().Be(address.City);
        resultAddress.AddressType.Should().Be(address.AddressType);
        resultAddress.Latitude.Should().Be(address.Latitude);
        resultAddress.Longitude.Should().Be(address.Longitude);
    }

    [Fact]
    public void MapToOfficeEntity_WhenAddressIsNull_Should_ReturnDefaultAddress()
    {
        ...
        result!.Address.Should().BeEquivalentTo(new AddressEntity());
    }

    [Fact]
    public void MapToOfficeEntity_WhenSourceIsNull_Should_ReturnNull()
    {
        var result = MapAccountDatabaseToAccountModel.MapToOffice((Office)null!);
