...
public class FavoriteServiceTests
{
    ...
    private FavoriteService CreateService()
    {
        ...
        return new FavoriteService(_favoriteRepository.Object, _roleEventPublisher.Object, _logger.Object, _roleRepository.Object);
    }
    ...
    public async Task Should_SetAccountFavorite_ReturnsOkResultAsync()
    {
        ...
        await service.SetFavoriteAsync(accountId: 1, contactId: 1, "Collaborator", true);
    ...
    public async Task SetAccountFavoriteAsync_Should_Throw_NotFoundException()
    {
        ...
        var result = async () => await service.SetFavoriteAsync(123, 123, "Collaborator", true);

        var exception = await Assert.ThrowsAsync<NotFoundException>(result);
        Assert.Equal(Errors.NotFoundRoleCode, exception.Code);
        Assert.Equal(Errors.NotFoundRoleMessage, exception.Message);

        _roleRepository.Verify(x => x.UpdateLastActivityDateAsync(123, 123, "Collaborator", It.IsAny<DateTime>()), Times.Never);
    }
    ...
    public async Task SetFavoriteAsync_Should_Call_PublishRoleCreatedEventAsync()
    {
        ...
        await service.SetFavoriteAsync(1, 1, "Collaborator", true);
